...
exports.sgbRoutes = exports.SgbRouter = void 0;
...
class SgbRouter {
    ...
    */
    ...
    note(req, res, next) {
        try {
            // Invoquer l'opération système (du DSS) dans le contrôleur GRASP
            let teacher_token = req.headers.token;
            let data = this.controller.note(teacher_token, parseInt(req.query.student_id), parseInt(req.query.course_id), req.query.type, parseInt(req.query.type_id), parseFloat(req.query.note));
            res.status(200)
                .send({
                message: 'Success',
                status: res.status,
    ...
    studentNote(req, res, next) {
        try {
            // Invoquer l'opération système (du DSS) dans le contrôleur GRASP
            let data = this.controller.studentNote(req.headers.token, parseInt(req.query.course), req.query.type, parseInt(req.query.type_id), parseFloat(req.query.note));
            this.generate_latency();
            res.status(200)
                .send({
                message: 'Success',
    ...
    latency(req, res, next) {
        this.router_latency = parseFloat(req.query.value); // Invoquer l'opération système (du DSS) dans le contrôleur GRASP
    ...
    generate_latency() {
        ...
        let latency = this.router_latency;
        let random = Math.random();
        let delay = +(random * latency * 1000).toFixed();
...
exports.SgbRouter = SgbRouter;
// exporter its configured Express.Router
exports.sgbRoutes = new SgbRouter();
exports.sgbRoutes.init();
